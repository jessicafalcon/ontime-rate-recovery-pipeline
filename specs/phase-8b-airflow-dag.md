...
`data/` writable (the landed shape; no bind mount). **Approved.**
...
|---|---|
| 1 | `tests/test_dag_structure.py::test_dag_tasks_are_the_pipeline_writing_steps_in_order` (manifest commands == `dbt build → write-back`, `PROFILE=tiny`, eval excluded — Amendment 1); `::test_dag_object_config_is_the_safe_scheduling` (loads the DAG with a **stubbed airflow** and asserts the real kwargs: `schedule=='@daily'`, `catchup is False`, `is_paused_upon_creation`, `max_active_runs==1`, `start_date` fixed); `::test_dag_object_tasks_edges_and_operator_kwargs` (two BashOperators in order, `cwd` = repo root, the `dbt_build → writeback` edge **with direction**); `::test_through_token_is_data_interval_end`; `tests/integration/test_int_airflow.py::test_dag_edges_and_operators` (the real DAG object against real Airflow, container) |
...
| 4 | `tests/test_backfill.py::test_three_through_landings_equal_the_union` (offline: the make/write-back chain over `BACKFILL_THROUGHS_TINY` into one DB == a union run, `SEND_SCHEDULE_SHA256_TINY`); `::test_backfill_interval_twice_is_a_noop`; `::test_computed_as_of_advances_when_scores_change` (the Amendment 2 precondition); `tests/integration/test_int_airflow.py::test_backfill_equals_union` (container, three explicit dates) |
| 5 | `tests/test_dag_structure.py::test_dag_object_config_is_the_safe_scheduling` (`max_active_runs==1` and `catchup is False` from the real DAG object); `tests/integration/test_int_airflow.py::test_backfill_runs_green` (the all-green three-interval run demonstrates the single-writer hand-off across processes); non-UTC identity carried by `tests/test_incremental.py::test_build_under_tokyo_is_identical` (the build is unchanged) |
...
|---|---|
| 1. **DAG == pipeline's writing steps, in order, no logic.** For all tasks, the DAG's ordered commands are `make pipeline`'s two writing steps (`dbt build → write-back`), wired `dbt_build >> writeback` (direction), every operator a `BashOperator`, the interval supplied only by Airflow templating; `eval` is excluded (Amendment 1). | `test_dag_tasks_are_the_pipeline_writing_steps_in_order`; `test_dag_object_config_is_the_safe_scheduling`; `test_dag_object_tasks_edges_and_operator_kwargs`; `test_through_token_is_data_interval_end`; `tests/integration/test_int_airflow.py::test_dag_edges_and_operators` (real DAG object, container) — declarative wiring pinned by the **stubbed-airflow DAG object** (values + edge direction) + the container object, no Python guard to mutate |
...
| 5. **Backfill ≡ union** (precondition: each score change advances `computed_as_of` — Amendment 2). For the three-interval backfill (intervals spaced ≤ `lookback_days`; landings monotone supersets adding forward-dated opens), the final `send_schedule` equals a union run and a re-run of an interval is a no-op. | `test_three_through_landings_equal_the_union`; `test_backfill_interval_twice_is_a_noop`; `test_computed_as_of_advances_when_scores_change`; `test_backfill_equals_union` (container) |
| 6. **Single-writer, no clock.** For all backfill runs, `data/<p>.duckdb` is written by one process at a time; nothing asserted reads a run id/timing/log; a non-UTC host build is identical. | `test_dag_object_config_is_the_safe_scheduling` (`max_active_runs==1`); `test_backfill_runs_green`; `test_build_under_tokyo_is_identical` |
...
- Untouched by contract: `generator/`, `fixtures/`, `dbt/`, `eval/`,
  ...
  gets a docstring-only update for Amendment 1),
