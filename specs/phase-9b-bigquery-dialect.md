...
5. **Live facts on `<project_id>` — facts.** (i) The `ontime-pipeline`
   ...
   iam service-accounts undelete <unique_id> --project=<project_id>`
   ...
   `<project_id>` with a user account; builds run as the SA through
...
- **W — An empty selection recreates empty tables (#7). SUPERSEDED BY X (round 3): `recreate` no longer exists.** Restores
  **invariant 3** (the landing is a function of `(fixture, THROUGH)`, the
  ...
  #7; SUPERSEDED BY X):** `recreate` is `create_table(…, exists_ok=True)` followed by a
...
Round 4 is the ONE scoped re-review (`review-round-3..HEAD`). Round 4's
residue (17 findings; approved 2026-08-30): test pins the fake could not
express (its rows now follow the URIs; the one-mechanism grep covers
`loader/cli.py`; the planted insert is built by column name), records of X
not yet propagated (invariant 3's statement, W/W′ superseded markers, §8,
CLAUDE, PHASES), and two accepted residuals — a two-landing test on the
fake proves only the fake (#13); the per-landing temp dir is one syscall and
making it lazy would re-open the seam (#15). No round 5: per the 9a round-8
protocol, later residue goes to BACKLOG with a trigger.
...
| 3. **The landing is a function of the fixture and `THROUGH`.** For all `(profile, through)`, each raw table on BigQuery is landed by exactly ONE mechanism — a load job with the generated contract schema and `WRITE_TRUNCATE` over exactly the files `loader.load.event_files` selects (a zero-byte object when none) — so its schema is always the contract and its rows exactly the selection's; nothing on the landing path reads prior table state or interpolates an identifier into SQL, hence a second landing is byte-identical, never appended (Amendment X; the two-landing property is pinned by the disposition + the read-less protocol — a two-landing test on the fake would prove only the fake, round 4 #13, accepted). | `tests/test_bq_landing.py::test_selects_the_same_files_as_the_duckdb_loader`, `::test_uploads_then_loads_with_the_generated_schema` (`WRITE_APPEND` → red; a hand-typed schema → red via `tests/test_dbt_sources.py::test_bq_schema_is_generated_from_the_contract`), `::test_empty_selection_lands_a_zero_byte_object_through_the_same_job` (X), `::test_landing_reads_nothing_and_uses_one_mechanism` (X: the protocol has no read call, the id follows `RAW_DATASET`, no query/DDL string in `loader/bq.py` OR `loader/cli.py` — round 4 #1), `::test_cli_empty_landing_succeeds_and_reports_zero` (round 4 #1/#2: the fake's rows follow the URIs) |
...
| 6. **A conflicting duplicate never lands silently on either dialect.** For all landings where one `insert_id` + clock triple carries two payloads, `dbt build` fails (a singular test over the source) — the loader's Python predicate, restated in SQL both dialects run. | `tests/test_staging.py::test_conflicting_duplicate_fails_the_dbt_test` (a planted conflict in a scratch DuckDB landing → the singular test red; the DuckDB loader's own refusal is bypassed by inserting after `load()`), `tests/integration/test_int_bigquery.py::test_planted_conflict_fails_on_bigquery` (the `json_value` form, live), `tests/test_bq_landing.py::test_duplicate_guard_keys_are_the_contract` |
| 7. **Cloud targets are gated and validated before any client exists; nothing offline reaches the network.** For all of `bq-load`, `dbt-build TARGET≠duckdb`, `test-int-bigquery`: `CONFIRM=yes` must have command-line origin and `PROJECT` must match `PROJECT_RE` before a storage/BigQuery client is constructed; the client factory is injectable and the offline suite injects fakes only. | `tests/test_makefile.py::test_bq_targets_confirm_from_command_line_only`, `::test_bq_targets_pass_project_as_one_literal`, `tests/test_bq_landing.py::test_no_client_is_built_before_validation`, `::test_parity_fixture_refuses_without_the_carried_gate` (V's own pin), `::test_int_bigquery_entry_validates_and_gates_before_pytest` |
...
- `loader/bq.py::bq_load invert-guard` (round 2 #9) — REMOVED with Amendment
  X: the `if not paths` guard is gone (the empty selection is the expression
  `paths or [empty]`, one code path), so the operator finds no `if`; the
  property is pinned by `test_empty_selection_lands_a_zero_byte_object_through_the_same_job`.
...
- [ ] Spec amendments — `specs/phase-2-staging.md`, `specs/phase-7-incremental.md`
      (renamed Evidence ids + Amendment U wording — the one sanctioned edit
      to merged specs, DECISIONS rounds 1–2); no later spec exists (Phase
      10's is finalized after 9b merges)
