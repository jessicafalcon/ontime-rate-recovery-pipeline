...
| 9. **The `.tf` tree is one allowlist.** For all edits to `infra/**/*.tf` or `infra/.terraform.lock.hcl`, the offline suite is red until `infra/MANIFEST.sha256` is rewritten by `make tf-freeze CONFIRM=yes` (its only writer, `$(origin)`-gated) — so every attribute is mutation-lethal, not only the ones a property pin names (P). | `tests/test_infra.py::test_tf_tree_matches_manifest` (any byte of any `.tf` → red), `::test_tf_freeze_requires_confirm_origin_and_writes_the_manifest`, `tests/test_makefile.py::test_tf_freeze_confirm_from_command_line_only` |
