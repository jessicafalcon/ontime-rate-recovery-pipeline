# Phase 2 — Staging on DuckDB (APPROVED 2026-08-25 — implemented, in review)
...
**Status: APPROVED 2026-08-25; implemented on `phase-2-staging`.** Three new runtime
...
3. **dbt SQL mutation operator — re-defer to Phase 3** (BACKLOG row, trigger
   ...
   (`tests/pins.py`, asserted by `tests/test_staging.py`: raw row count, staged
   count, their difference = `DEDUPE_COUNT`). An operator that deletes a `qualify`/`where` clause would
...
- `make review-gate SPEC=…` — offline suite (loader, pins via an in-process
  ...
  five-macro greps, Makefile origin tests, truth isolation over `dbt/` and
...
| 2 | `tests/test_staging.py::test_pins_are_reproduced` (every constant in `tests/pins.py` vs the built tables), dbt unit test `stg_events_json_null_error_code_is_sql_null` + singular `assert_error_code_only_on_upload_failed` (the `error_code` null pin), `::test_upload_delay_sign_and_range_match_pins`, `::test_every_event_has_a_tz_and_undelivered_prompts_match_pin`, `tests/test_loader.py::test_loader_globs_every_raw_file` (10 files, 970 rows, incl. `events_2026-01-04.jsonl`) |
| 3 | dbt unit test `stg_events_dedupes_insert_id` (`dbt/models/staging/schema.yml`), dbt data test `unique` on `stg_events.insert_id`, `tests/test_staging.py::test_dedupe_count_matches_pin`, `tests/test_loader.py::test_duplicate_insert_id_across_files_is_loaded_twice` (raw keeps both; staging them to one is the unit test's job) |
...
| 5 | `tests/test_dbt_conventions.py::test_exactly_five_dispatch_macros`, `::test_each_macro_has_duckdb_and_bigquery_bodies` (renamed in Phase 9b), `::test_no_default_dispatch_body`, `::test_no_clock_call_in_any_model_or_macro`, `::test_session_zone_is_pinned_in_profile_and_macro`, `::test_no_dbt_packages`, `::test_every_model_has_description_and_a_test`; `tests/test_staging.py::test_build_under_a_non_utc_host_zone_is_identical` |
| 6 | `tests/test_dbt_sources.py::test_committed_sources_equal_regeneration`, `::test_hand_edit_is_detected` (tmp copy with one column removed → not equal), `::test_no_unique_test_on_raw_insert_id`; `make gen-sources` output line `gen-sources OK: 3 files, 0 stale (3 since Phase 9b added `bq_schema.json`)` |

## Invariants (REQUIRED)

| Invariant ("for all …, … holds") | Falsified by (scenario test) |
|---|---|
| 1. For all `insert_id`, exactly one `stg_events` row, regardless of how many raw rows carry it and which upload-date files they land in; the surviving row is the one with the earliest `server_upload_time`, then `server_received_time`, then `client_event_time` — content-derived, never file or load order; rows tying on all three clocks but differing in `event_properties` are refused at landing, never ordered (amendment 3). | dbt unit test `stg_events_dedupes_insert_id` (6 raw rows, 3 ids: copies split across two `server_upload_time` dates, one copy re-received later, one pair tying on both server clocks and differing in `client_event_time` → 3 rows, the earliest on each key); `tests/test_loader.py::test_conflicting_duplicate_is_refused_at_landing`, `::test_distinct_insert_ids_sharing_all_clocks_are_not_a_conflict` (the conflict key includes `insert_id`); `unique` on `stg_events.insert_id`; `tests/test_loader.py::test_duplicate_insert_id_across_files_is_loaded_twice`; `tests/test_staging.py::test_dedupe_count_matches_pin` |
...
| 3. For all models and macros, no call reads the clock (`current_timestamp`, `now()`, `current_date`, `get_current_timestamp`, `run_started_at`), no source has a freshness block, and no build reaches the network (dbt telemetry off); two builds of the same fixture produce identical staged tables. | `tests/test_dbt_conventions.py::test_no_clock_call_in_any_model_or_macro` (grep over `dbt/models/**`, `dbt/macros/**`, `dbt/tests/**`; a planted `now()` in a tmp copy is found), `::test_no_freshness_block`, `::test_telemetry_is_off`, `::test_session_zone_is_pinned_in_profile_and_macro`, `tests/test_staging.py::test_two_builds_are_identical` (row hashes of both staging tables, build twice into two `tmp_path` dbs), `::test_build_under_a_non_utc_host_zone_is_identical` (second process, `TZ=Asia/Tokyo`) |
| 4. For all dialect-divergent SQL, it goes through exactly five `adapter.dispatch` macros; every macro has a `duckdb__` implementation and a `bigquery__` body (a stub that raised until Phase 9b; real since — the partition seam's BigQuery half is the adapter's native strategy, Amendment U) — never a `default__` body a new adapter could silently fall into. | `tests/test_dbt_conventions.py::test_exactly_five_dispatch_macros` (parses `dbt/macros/*.sql`; `adapter.dispatch` nowhere else), `::test_each_macro_has_duckdb_and_bigquery_bodies` (renamed in Phase 9b), `::test_no_default_dispatch_body` |
| 5. For all constants in `tests/pins.py`, a build from `fixtures/tiny/` reproduces them; a pin that drifts is a red test, never a rewritten number. | `tests/test_staging.py::test_pins_are_reproduced`, `::test_upload_delay_sign_and_range_match_pins` (sign of `upload_delay_seconds`: a swapped `timestamp_diff` argument order is red), `::test_every_event_has_a_tz_and_undelivered_prompts_match_pin`; `tests/test_loader.py::test_loader_globs_every_raw_file`; dbt unit tests `stg_events_upload_delay_sign`, `stg_events_json_null_error_code_is_sql_null` (JSON `null` → SQL NULL, reconciliation item 4; the data test `assert_error_code_only_on_upload_failed` pins it on the fixture) |
| 6. For all raw tables, the source tests are `not_null` on every non-Optional field, `accepted_values` on `event_type` over `EventType`, `unique` on `dim_user (user_id, valid_from)`; NO `unique` on `raw.events.insert_id` and NO `freshness`. | `tests/test_dbt_sources.py::test_no_unique_test_on_raw_insert_id`, `::test_source_tests_cover_every_required_column`, `tests/test_dbt_conventions.py::test_no_freshness_block`, `::test_singular_tests_exist_and_target_their_relation` (the composite key is `dbt/tests/assert_dim_user_key_unique.sql`), `::test_schema_event_types_equal_the_contract` |
...
| 8. For all `load` / `dbt-build` / `drop-db` runs, `PROFILE` and `TARGET` are validated `[a-z0-9_]+` before any path is derived; `drop-db` deletes only `data/<profile>.duckdb` and its `.wal` and only with `CONFIRM=yes` from the command line; a non-`duckdb` `TARGET` needs the same `CONFIRM`; `load` names its source, refuses on `MANIFEST.sha256` drift, and is idempotent (raw tables recreated, never appended). | `tests/test_makefile.py::test_load_and_dbt_build_pass_profile_and_target_as_one_literal`, `::test_drop_db_requires_confirm_from_the_command_line`, `tests/test_loader.py::test_profile_and_target_are_validated`, `::test_cloud_target_requires_confirm_from_the_command_line`, `::test_load_reports_its_source_and_refuses_manifest_drift`, `::test_manifest_check_reads_only_raw_and_dims`, `::test_load_twice_gives_the_same_row_count`, `::test_drop_db_removes_only_the_named_file` (plants a `.wal`) |
| 9. For all files under `dbt/` and `loader/`, the word truth does not appear, and the loader reads bytes only under `raw/` and `dims/` (invariant 2 of Phase 1, now live for two new pipeline dirs — word and byte level). | `tests/test_truth_isolation.py::test_pipeline_dirs_never_mention_truth`, `::test_pipeline_dirs_are_derived_from_the_tree`, `tests/test_loader.py::test_manifest_check_reads_only_raw_and_dims` |

```mutations
loader/load.py::event_files                     constant-return:[]
loader/load.py::manifest_drift                  constant-return:[]
loader/load.py::conflicting_duplicates          constant-return:[]
...
**Amendment 2 (review round 1, 2026-08-25 — APPROVED, implemented in
`phase-2:` commits after `785dacc`): five design changes.** Each names the invariant it restores.
...
- **`stg_prompts` is one row per `prompt_id`** (reconciliation item 6): from
  ...
  `client_event_time`, then `insert_id`; the winner is exposed as
  `delivered_insert_id` so the tie-break is observable — unit test
  `stg_prompts_takes_first_delivery_receipt` ties two receipts) — the grain
  Phase 3 attributes on.
...
- [x] `DECISIONS.md` — Phase 2 entry: dim_user as loaded source, generated
      sources, dedupe tie-break, stub-raises macros, single entry point,
      `stg_prompts` grain, SQL-mutation re-deferral
- [x] `docs/PHASES.md` — Phase 2 Done-when as landed; "Delivered" paragraph;
      source-test wording (no `unique` on raw `insert_id`)
- [x] `CLAUDE.md` — Current status; Commands (`load`, `dbt-build`, `drop-db`,
      `gen-sources`); Repo map (`dbt/`, `loader/` real); allowlist unchanged
      (three Phase 2 packages landed); BACKLOG count
- [x] `docs/ARCHITECTURE.md` — §3 / §3.3 "dim seed file (loaded as a source)";
      §8 Gotchas for every dbt-duckdb / DuckDB JSON surprise found live
- [x] `BACKLOG.md` — strike "Raw DDL and dbt sources.yml …" (DONE Phase 2);
      re-defer "Mutation sweep has no operator for dbt SQL" with trigger
      Phase 3; rows opened at review round 1: `partition_overwrite` unrendered
      (Phase 7), two cohort keys on `stg_prompts` (Phase 3), row-hash content
      pin (Phase 3 `expected/`)
...
| `make dbt-build PROFILE= TARGET= CONFIRM=` | PROFILE empty refused; TARGET empty → `duckdb` | refused | refused | PROFILE/TARGET validated identically; `CONFIRM=yes` from the environment is NOT accepted for a cloud target | any `TARGET` other than `duckdb` refuses unless `$(origin CONFIRM)` = `command line` and value `yes` — the only guard: a developer's ADC may well exist, and Phase 9 replaces the raising stubs with real bodies | `tests/test_makefile.py::test_load_and_dbt_build_pass_profile_and_target_as_one_literal`, `tests/test_loader.py::test_cloud_target_requires_confirm_from_the_command_line`, `::test_profile_and_target_are_validated` |
| `make drop-db PROFILE= CONFIRM=` | refused; without CONFIRM prints `drop-db: refused — pass CONFIRM=yes on the command line`, exit 2, no delete | refused | refused | `CONFIRM=yes` from the environment is NOT accepted | recipe passes `$(origin CONFIRM)`; Python accepts only `command line`; removes `data/<p>.duckdb` and `data/<p>.duckdb.wal`, nothing else | `tests/test_makefile.py::test_drop_db_requires_confirm_from_the_command_line`, `tests/test_loader.py::test_drop_db_removes_only_the_named_file` |
...
removes gitignored files that `load` recreates from the fixture. `load`'s
fallback to `data/out/<p>/` is printed, never silent, and a frozen profile
is verified against its manifest before a byte is loaded.
...
- **code-reviewer** (triggered — code in Scope): no clock on a data path, the
  ...
  description + a test, the five-macro count, allowlist (three packages, no
...
- **coherence-auditor** at exit (mandatory): §3/§3.3 no longer say "seed";
  ...
  four new targets (`load`, `dbt-build`, `drop-db`, `gen-sources`).
...
- BigQuery bodies for the five macros — Phase 9; the landing conflict guard on
  the BigQuery path — BACKLOG (review round 3), trigger Phase 9.
