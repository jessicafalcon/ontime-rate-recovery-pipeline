...
|---|---|
| 1. For all `insert_id`, exactly one `stg_events` row, regardless of how many raw rows carry it and which upload-date files they land in; the surviving row is the one with the earliest `server_upload_time`, then `server_received_time`, then `client_event_time`, then the lowest `md5(event_properties)` — a total, content-derived order, never file or load order (amendment 2.5). | dbt unit test `stg_events_dedupes_insert_id` (6 raw rows, 3 ids: copies split across two `server_upload_time` dates, one copy re-received later, one exact three-clock tie differing only in payload → 3 rows, the earlier / lower-hash ones); `unique` on `stg_events.insert_id`; `tests/test_loader.py::test_duplicate_insert_id_across_files_is_loaded_twice`; `tests/test_staging.py::test_dedupe_count_matches_pin` |
