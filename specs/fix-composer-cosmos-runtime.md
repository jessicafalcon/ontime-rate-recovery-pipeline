...
| 6 | `tests/test_serving_image.py::test_image_context_is_serving_and_landing_only` (context copies only `serving/`+`landing/`+their imports, no `truth/`; `test_truth_isolation.py` covers the Dockerfile); `git diff` shows `pipeline_dag.py`/`tasks.py`/`fixtures/` unchanged |
