...
def render_block(rows: dict[str, object]) -> str:
    ...
    headline = (
        f"**On the 2,000-user profile the recommended schedule lifts the simulated "
        f"on-time rate from {base:.0%} to {reco:.0%} "
        f"({(reco - base) * 100:+.0f} points).** Every number here is rendered by "
