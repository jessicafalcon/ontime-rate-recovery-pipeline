...
Every prompt×user gets exactly one label; `unattributed` is an explicit
...
moment (`MAX_USER_SHIFT_MIN`). This lowers the theoretical
...
warehouses. Python is reserved for `eval/`.
...
- **Stubbed with documented production path:** Amplitude export (event generator
  ...
  `cohort_id`, `send_hour_local`, `send_minute_local`, `tz`, `confidence`,
  `model_version`, `computed_as_of`, `written_at`; the
...
- **Cross-warehouse macros are limited to the known divergences:** JSON
  ...
  `dbt build` on DuckDB per PR from Phase 2 so dialect drift is caught early.
...
- **Guardrails as a deliverable:** budget alerts ($50 / $150), isolated Composer
  module, one-command teardown, all documented in `docs/DEPLOYMENT.md`.
...
lateness causes are **separable** and the timing gap is **recoverable**.
Validation results on the locked dataset (8,000 users, ~86k responses; a
pre-review two-cause generator — Phase 1 re-validates with the five labels,
adding `delivery_fault` and `unattributed` shares):
...
The validation is reproducible once Phase 1 lands the generator: `make seed
PROFILE=medium` with the fixed seed regenerates it; `fixtures/tiny/` (~20
users) carries the golden diff only, not these numbers.
...
| **2 — Staging on DuckDB** | A | dbt project; loader; `stg_*` (dedupe on `insert_id`, tz → local); source tests; the five dispatch macros; **CI: `dbt build`**. |
...
| **11 — Composer module (written, not applied)** | Composer | Isolated module behind `enable_composer`; DAG upload; `docs/DEPLOYMENT.md`. Zero meter. |
...
|---|-----------|------------|
| 1 | Per-user send times dissolve the simultaneous "shared moment" that defines the product category | Cohort-band optimization; bounded per-user shift within the cohort moment (§2) |
