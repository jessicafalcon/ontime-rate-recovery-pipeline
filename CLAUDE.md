...
- `make readme [WRITE=yes]` *(Phase 13)* — the README first-screen block
  ...
  readme` from `tests/pins.py` + the committed `docs/RESULTS.md` blocks — no
  number typed by hand. Check mode diffs both byte-for-byte (exit 1 on drift);
...
Cosmos / `KubernetesPodOperator`, superseding Option A; a real-scale cost run),
each named as its own future branch; re-stated the tfstate confidentiality half
