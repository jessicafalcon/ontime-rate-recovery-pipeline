...
got a row in the same commit as this file.
...
|---|---|---|---|---|---|
| 1 | `fix/front-door` | Reframe, not hide: strip phase/amendment language from README and INSIGHT prose; add `docs/PROCESS.md` — one page on the AI-assisted spec-and-review loop and what kept it honest (pins, mutation sweep, truth-isolation grep, frozen fixture) | `make check-docs`; coherence-auditor over the changed docs | 1 day | none |
| 2 | `fix/tf-remote-state` | Bootstrap the state bucket once by hand, uncomment the drafted `gcs` backend in `infra/main.tf`, migrate state, `tf-freeze` | `make tf-validate`; a `tf-plan` reading remote state | ½ day | cents |
| 3 | `fix/scores-dim-current` | `scores_send_time` reads `ref('dim_user_current')` for the open dim row instead of the raw source | the three goldens unchanged byte-for-byte (`make scores-golden`, `make report`, `make attribution-golden`) | ½ day | none |
| 4 | `fix/holdout-eval` | Temporal holdout: build medium with the landing cut at day 20 (`THROUGH`), serve, then a new `eval/cli.py holdout` command reads the RAW organic opens of days 21–30 (raw, never truth) and scores served-hour vs cohort send-hour on held-out opens: share inside the reachable window, circular distance to the nearest open; pinned in `tests/pins.py`, a generated block in `docs/RESULTS.md` beside the simulation | the new pin under `make test`; check-mode block diff | 2 days | none |
| 5 | `fix/large-profile` | A `large` profile (200,000 users × 30 days ≈ 35 M events ≈ 10 GB); the generator sharded by a derived `(SEED, shard)` seed, emit order preserved within a shard; ask-first: one full `TARGET=bigquery` build and one `THROUGH` incremental build; RESULTS.md records bytes scanned per model, slot-ms, wall clock, dollars, and the pruning proof | the recorded table; DuckDB goldens untouched | 3–4 days | single-digit $ |
| 6 | `fix/append-landing` | Raw becomes append-only, partitioned by upload date; each export file loads once into its partition; `THROUGH` becomes a partition predicate; the writer emits gzipped hourly files like the real Amplitude export. A write-path change: one-paragraph amendment first | `make test-int-bigquery` byte parity; a re-land writes 0 new rows | 3 days | cents |
...
| 8 | `fix/ci-bigquery-parity` | `enable_ci_wif=true` apply; a `workflow_dispatch` job running the existing `test-int-bigquery` | a green dispatched run | 1 day | cents |
...
are published** (item 5), **The make-based DAG cannot run on Composer** (item 7),
**Cross-warehouse dialect drift is caught only on DuckDB in CI** (item 8). New
rows for items 1, 3, 4 and 6.

## The one-week cut

Decided 2026-09-01: build **items 1, 2, 3 and 5** first, in that order. The
...
- Item 4 is a feature outside `docs/ARCHITECTURE.md` § scope; its branch opens
  with a one-paragraph amendment, approved in the same review.
